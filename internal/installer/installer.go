...
var resolveTwoPartInstallTargetForInstall = resolveTwoPartInstallTarget
var resolveBareCachedSkillInputForInstall = resolveBareCachedSkillInput

func refreshStaleCachedRepoForInstall(repoName string, opts InstallOptions) {
	reposCache, cacheErr := cache.NewReposCache()
	if cacheErr != nil || reposCache == nil || !reposCache.HasRepo(repoName) {
		return
	}
...
func printBareCachedSkillAmbiguity(input string) {
	reposCache, err := cache.NewReposCache()
	if err != nil {
		return
	}
...
func Install(input string, opts InstallOptions) error {
	...
	if directIsSafeToUse {
		...
		parts := strings.Split(input, "/")
		if len(parts) == 2 {
			refreshStaleCachedRepoForInstall(parts[0], opts)
			twoPartTarget, twoPartOK, err := resolveTwoPartInstallTargetForInstall(input, opts)
			if err != nil {
				return err
			}
			if twoPartOK {
				repoURL = twoPartTarget.repoURL
				branch = twoPartTarget.branch
	...
	if !strings.Contains(input, "/") && !strings.HasPrefix(input, "http://") && !strings.HasPrefix(input, "https://") && !strings.HasPrefix(input, "git") {
		...
		resolvedInput, resolvedFromCache, err := resolveBareCachedSkillInputForInstall(input)
		if err != nil {
			printBareCachedSkillAmbiguity(input)
			return err
		}
		if resolvedFromCache {
			repoName := strings.SplitN(resolvedInput, "/", 2)[0]
			ui.Debug(fmt.Sprintf("Found skill '%s' in local cache (repo: %s)", input, repoName))
			recurseOpts := opts
