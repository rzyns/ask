...
func TestShowHermesAgentSkillsStopsOnMalformedLockFile(t *testing.T) {
	root := t.TempDir()
	writeHermesListSkill(t, root, "managed-skill", "managed-skill", "Managed description", "")
	...
	if err := os.WriteFile("ask.lock", []byte("version: [unterminated\n"), 0o600); err != nil {
		t.Fatalf("write malformed lock file: %v", err)
	}

	items := showHermesAgentSkills("hermes", root, "Project", false, true)
	if len(items) != 0 {
		t.Fatalf("showHermesAgentSkills returned %#v, want no items when lock file is malformed", items)
	}
}
